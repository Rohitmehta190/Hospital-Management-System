...
import { FiUsers, FiCalendar, FiActivity, FiTrendingUp, FiAlertCircle, FiDatabase, FiServer, FiHardDrive, FiWifi, FiCpu, FiCheckCircle, FiPlus, FiDownload, FiSettings, FiUserPlus, FiRefreshCw, FiFileText, FiShield, FiClock, FiDollarSign, FiUser } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        console.log('Admin Data Fetched:', { patients, doctors, appointments });
        ...
        setStats({
          ...
          totalRevenue: appointments.length * 75, // Estimated revenue
          systemHealth: 95, // Would come from system monitoring
          activeUsers: patients.length + doctors.length + 10 // Estimated active users
        });
        ...
        if (patients.length > 0) {
          ...
          activities.push({
            id: Date.now() + 1,
            type: 'user',
            message: `New patient registered: ${latestPatient.first_name} ${latestPatient.last_name}`,
            time: 'Just now',
            icon: FiUser,
            color: 'text-blue-600'
          });
        ...
        if (appointments.length > 0) {
          ...
          activities.push({
            id: Date.now() + 2,
            type: 'appointment',
            message: `Appointment scheduled: ${latestAppointment.patient_name} with Dr. ${latestAppointment.doctor_name}`,
            time: '2 minutes ago',
            icon: FiCalendar,
        ...
        activities.push({
          id: Date.now() + 3,
          type: 'system',
          message: 'System data synchronized successfully',
          time: '5 minutes ago',
          icon: FiCheckCircle,
        ...
        if (doctors.length > 0) {
          activities.push({
            id: Date.now() + 4,
            type: 'system',
            message: `${doctors.length} doctors currently registered`,
            time: '10 minutes ago',
            icon: FiDatabase,
        ...
        if (patients.length > 1000) {
          alerts.push({
            id: Date.now() + 1,
            level: 'warning',
            message: `Patient database growing rapidly: ${patients.length} total patients`,
            time: '15 minutes ago'
          });
        }
        ...
        if (appointments.length > 50) {
          alerts.push({
            id: Date.now() + 2,
            level: 'info',
            message: `High appointment volume: ${appointments.length} total appointments`,
            time: '1 hour ago'
          });
        }
